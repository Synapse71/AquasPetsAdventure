...
export function AdventureItems({ inventory, empty = '暂无物品', onPick, movable, slots = 0, room, rows = false, metric = 'weight' }: {
  inventory: Inventory; empty?: string; onPick?: (id: string, quantity: number) => void; movable?: Inventory; slots?: number;
  ...
  return <div className="ap-items" aria-label="物品格子" onScroll={() => setTip(undefined)}>
    {Array.from({ length: Math.max(cells.length, Math.min(slots, 120)) }, (_, i) => {
      ...
      return <button key={`${c.id}-${i}`} className={`ap-item${onPick && quantity ? ' movable' : ''}${room && !quantity ? ' full' : ''}${movable && quantity ? ' fresh' : ''}${movable && !quantity ? ' old' : ''}`} style={{ '--item-color': colors[item.rarity] } as CSSProperties} aria-label={`${item.name} ×${c.quantity}`} data-description={title} aria-disabled={!!onPick && !quantity} onMouseEnter={e => { const r=e.currentTarget.getBoundingClientRect(); setTip({id:c.id,x:Math.max(6,Math.min(window.innerWidth-240,r.left)),y:Math.max(6,Math.min(window.innerHeight-160,r.bottom+6))}); }} onMouseLeave={() => setTip(undefined)} onFocus={e => { const r=e.currentTarget.getBoundingClientRect(); setTip({id:c.id,x:Math.max(6,Math.min(window.innerWidth-240,r.left)),y:Math.max(6,Math.min(window.innerHeight-160,r.bottom+6))}); }} onBlur={() => setTip(undefined)} onClick={() => {setTip(undefined); if(quantity) onPick?.(c.id, 1);}} onContextMenu={e => { e.preventDefault(); setTip(undefined); if (quantity) onPick?.(c.id, quantity); }}>
        {url ? <img src={url} alt="" draggable={false} /> : <span>◇</span>}{c.quantity>1 && <b>{c.quantity}</b>}
      </button>;
    })}
    {!cells.length && !slots && <p className="ap-muted">{empty}</p>}
    {tip && catalog.items[tip.id] && <div className="ap-item-tip" role="tooltip" style={{left:tip.x,top:tip.y}}><strong>{catalog.items[tip.id].name}</strong><p>{RARITY_LABELS[catalog.items[tip.id].rarity]}</p>{catalog.items[tip.id].description && <em>{catalog.items[tip.id].description}</em>}</div>}
  </div>;
}
