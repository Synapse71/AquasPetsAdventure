  ...
  return <div className="simple-panel settings-panel">
    ...
    <div className="sp-content st-content">{tab === 'log' ? activity : tab === 'about' ? <div className="st-about">
      ...
      <section className="st-section"><h2>桌宠</h2>
        ...
        <label className="st-row"><span className="st-label">始终置顶</span><span className="st-description">{desktop?.alwaysOnTop === false ? '已关闭：桌宠会被其他窗口挡住，可从托盘菜单「呼到最前」找回' : '让桌宠保持在其他窗口上方'}</span><input className="st-switch" aria-label="始终置顶" type="checkbox" checked={desktop?.alwaysOnTop ?? true} disabled={!desktop} onChange={e => apply({alwaysOnTop:e.target.checked})} /></label>
