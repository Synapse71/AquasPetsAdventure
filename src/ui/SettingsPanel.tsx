...
export function SettingsPanel({ game, onReset, onClose, active, activity }: {
  ...
  const apply = (value: {canvas?:number;alwaysOnTop?:boolean}) => {
  ...
  const size = desktop?.canvas ?? 300, bodyWidth = size * .47, bubble = (bodyWidth - 16) / 5;
  return <div className="simple-panel settings-panel">
    ...
    <div className="sp-content st-content">{tab === 'log' ? activity : tab === 'about' ? <div className="st-about">
      ...
      <section className="st-section"><h2>桌宠</h2>
        <label className="st-row"><span className="st-label">桌宠大小</span><span className="st-description">同步调整本体与气泡菜单</span><input aria-label="桌宠大小" type="range" min={260} max={420} step={10} disabled={!desktop} value={size} onChange={e => apply({canvas:Number(e.target.value)})} /><output>{size}px</output></label>
        <div className="st-preview"><PetPortrait /><div className="st-preview-info"><span className="st-ruler" style={{width:bodyWidth}}>菜单宽 {bodyWidth.toFixed(1)}px</span>
          <div className="st-menu-preview" aria-label="真实像素气泡预览" style={{width:bodyWidth}}>{PET_MENUS.map(menu => <span key={menu.id} style={{width:bubble,height:bubble}}><img src={menuIcon(menu.icon)} alt={menu.label} /></span>)}</div>
          <span className={bubble < 22 ? 'sp-warning' : 'sp-muted'}>气泡直径 {bubble.toFixed(1)}px{bubble < 22 ? ' · 小于建议的 22px，图标可能不易辨认' : ' · 与桌面实际气泡尺寸一致'}</span>
