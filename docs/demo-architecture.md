...
- `MapForm.tsx`：地图基础信息（名称、编号、起始节点、情报阈值）直接在页面上编辑，跟其他表单一样由底部「保存这条记录」落盘。
  节点则走**拓扑图入口**：从当前草稿自动生成入口向右分层的有向拓扑预览，不可达节点和悬空路径会在图中提示；点击图上任一节点弹出该节点的编辑浮层，
  里面是名称、角色、掉落表、白/黑名单、事件池、首次撤离奖励和单向路线。浮层改的是节点副本，点「保存」一次就写进 catalog 草稿并关闭，
  「取消」/ Esc / 点背景丢弃改动（有改动时会先确认）。新增节点后会自动打开它的浮层，删除节点也在浮层里完成。
  节点保存会把整条地图记录（含尚未保存的基础信息改动）一起落盘——这是同一条记录，无法只存节点。
- `mapNodeDraft.ts`：上面这套「草稿 → 保存」的纯逻辑（脏检查、写回节点、删除节点连带清路线、入口层级 BFS、节点角色判定），不依赖 React，由 `mapNodeDraft.test.ts` 覆盖。
...
- 地图编辑的真交互回归跑 `npm run test:config-map`：它起一个 Vite dev server 和 Electron 壳指向 `?config=1`，用 CDP 派发真实鼠标事件做命中测试，覆盖点节点开浮层、保存落盘、刷新保留、取消丢弃、新增/删除节点，以及长表单时浮层必须夹在视口内（保存栏不能被顶出屏幕）。
...
- `xpMultiplier` 是本图的经验系数，节点经验和战利品经验都乘它，省略即为 1。越难的地图给得越高，否则玩家没有推进的成长动力。
- 单轮深度应遵守系统设计文档中的进程约束。
...
| `MapDefinition.xpMultiplier` | 本图经验系数，省略即为 1 |
...
- 升级曲线严格递增，满级后不涨级也不囤积经验；战利品经验按稀有度与件数累加；地图系数同时乘节点经验和战利品经验；就地卖掉的算经验、丢弃的不算；溃败只给节点经验的一半且没有战利品经验。
